...
3. **Check for existing designs**: Before downloading, check if `.stitch/designs/{page}.html` and `.stitch/designs/{page}.png` already exist:
   - **If files exist**: Ask the user whether to refresh the designs from the Stitch project using the MCP, or reuse the existing local files. **You MUST ask â€” do not assume.**  Only re-download if the user confirms.
...
5. **Visual audit**: Review the downloaded screenshot (`.stitch/designs/{page}.png`) to confirm design intent and layout details. **You MUST view each screenshot** â€” do not proceed based on assumptions about the design.
...
- âŒ Reading `.stitch/designs/*.html` directly without calling MCP `get_screen` first.
- âŒ Skipping the `fetch-stitch.sh` download script.
- âŒ Not asking the user when existing files are found.
- âŒ Skipping the visual audit of `.png` screenshots.
- âŒ Failing to generate or update `.stitch/metadata.json` and its `Last Sync Time` field upon syncing.
...
- âŒ Using `style-guide.json` as-is without verifying it matches the current project.
- âŒ Using hardcoded hex values in components instead of theme-mapped classes.
...
* **Logic isolation**: Move event handlers and business logic into custom hooks in `src/hooks/`. Examples: pagination logic â†’ `usePagination`, filtering â†’ `useFilter`.
* **Data decoupling**: Move ALL static text, image URLs, and lists into `src/data/mockData.ts`. No hardcoded content in components.
* **Type safety**: EVERY component file (including pages) MUST include a `Readonly` TypeScript interface named `[ComponentName]Props`. The validator checks for this â€” files without a Props interface will FAIL validation.
...
* **Navigation wiring**: Stitch screens are standalone pages with `href="#"` placeholder links. When building a multi-page React app:
    ...
    * **Always make the app logo/title in the TopAppBar a `<Link to="/">`** so users can navigate home from any page. This is critical because Stitch bottom nav bars use `md:hidden` and are invisible on desktop â€” without a clickable logo, desktop users have no way to return to the home page.
...
- âŒ Putting all UI in a single monolithic page file.
- âŒ Inline event handlers or business logic without hooks.
- âŒ Hardcoding text, URLs, or data in component files.
- âŒ Components without a `[Name]Props` interface.
- âŒ Using hex color values instead of theme tokens.
- âŒ Leaving `href="#"` links unconverted.
...
- âŒ Commencing dev server start or browser audits without user consent.
- âŒ Declaring task "done" without verifying code compiles.

## Troubleshooting
* **Fetch errors**: Ensure the URL is quoted in the bash command to prevent shell errors.
* **Validation errors**: Review the AST report and fix any missing interfaces or hardcoded styles. The most common failure is a missing `Props` interface â€” every component (including pages) needs one.
