...
- âŒ Hardcoding color hex codes or rgba strings directly inside component StyleSheet declarations.
- âŒ Using theme tokens from a previous project without extracting them from the new design.
- âŒ Skipping the creation/update of `src/theme.ts`.
...
* **Type safety**: EVERY component file (including screens) MUST export a TypeScript interface named `[ComponentName]Props` with `readonly` property modifiers. The validator requires the interface to be **exported** â€” files without an exported Props interface will FAIL validation.
...
- âŒ Putting all UI in a single monolithic screen file.
- âŒ Using HTML tags (like `div`, `span`, `p`) instead of React Native components.
- âŒ Inline event handlers or business logic without custom hooks.
- âŒ Hardcoding text, URLs, or colors in component files.
- âŒ Components without an **exported** `[Name]Props` interface.
- âŒ Using raw hex color values or rgba strings in `StyleSheet.create()`.
...
- âŒ Launching packagers or simulators without user consent.
