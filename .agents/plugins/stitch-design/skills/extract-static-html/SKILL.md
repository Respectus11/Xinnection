...
| `--title` | â€” | Override page title (set to the route path, e.g. `/dashboard` or `/settings/profile`) |
| `--auth-script` | â€” | Path to a JS/TS module that exports a default `async (page) => void` function for authentication |
...
- Converts `<img>` `src` **and `srcset`** â†’ base64 data URIs (skips external fonts)
