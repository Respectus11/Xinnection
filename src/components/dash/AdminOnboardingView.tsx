...
export function AdminOnboardingView() {
  ...
  const [nodeAlias, setNodeAlias] = useState("Station-04-Primary-YubiKey");
  const [requireBiometric, setRequireBiometric] = useState(true);
  const [registerBackup, setRegisterBackup] = useState(false);
  ...
  const simulateKeyTap = () => {
    if (!isAttested) {
      ...
      setTimeout(() => {
        ...
        setIsAttested(true);
        showToast("YubiKey FIDO2 token attestation verified.");
      }, 800);
    } else {
      setIsAttested(false);
      showToast("Token attestation reset.");
    }
  };

  const handleAttestAndContinue = () => {
    if (!isAttested) {
      setIsSimulating(true);
      setTimeout(() => {
        setIsSimulating(false);
        setIsAttested(true);
        showToast("Auto-attestation complete. Enrolling node...");
        setTimeout(() => {
          router.push("/admin");
        }, 1200);
      }, 700);
    ...
    showToast("Cryptographic node enrolled. Redirecting to admin console...");
    setTimeout(() => {
      router.push("/admin");
    }, 1000);
  };
  ...
  return (
    <div className="bg-canvas-deep text-starlight-white font-body-md min-h-screen flex flex-col justify-between selection:bg-primary-container selection:text-starlight-white relative overflow-x-hidden">
      ...
      {showHelpModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 backdrop-blur-sm p-4">
          <div className="bg-elevated-onyx border border-white/10 rounded-2xl max-w-md w-full p-6 space-y-4 shadow-2xl">
            <h3 className="text-headline-sm font-semibold text-starlight-white">FIDO2 Hardware Key Enrollment</h3>
            <p className="text-muted-silver text-sm leading-relaxed">
              Xinnection requires physical hardware attestation (YubiKey 5 Series, SoloKeys, or Nitrokey) for responder administrative operations to prevent credential theft and session hijacking.
            </p>
      ...
      <header className="relative z-10 w-full px-4 sm:px-6 lg:px-8 py-space-sm bg-elevated-onyx/80 backdrop-blur-md flex items-center justify-between shadow-sm">
        <div className="flex items-center gap-space-md">
          <Link href="/" className="flex items-center gap-space-sm hover:opacity-90">
        ...
        <div className="flex items-center gap-3 sm:gap-space-md">
          <button 
            ...
            className="flex items-center gap-space-xs text-body-sm font-body-sm text-muted-silver hover:text-starlight-white transition-colors"
            type="button"
          >
      ...
      <main className="relative z-10 flex-1 flex items-center justify-center p-4 sm:p-6 lg:p-8">
        <div className="w-full max-w-3xl bg-elevated-onyx border border-zinc-800/80 rounded-2xl shadow-2xl p-6 sm:p-8 relative overflow-hidden backdrop-blur-xl">
          ...
          <div className="mb-space-lg">
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-blue-950/60 border border-blue-500/30 text-blue-400 text-mono-data font-mono-data mb-space-sm tracking-wider text-xs">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-space-lg p-2 bg-zinc-950/70 border border-zinc-800/80 rounded-xl">
            ...
            <div className="flex items-center gap-3 p-2.5 rounded-lg bg-blue-950/30 border border-blue-500/50 shadow-sm relative">
              ...
              <div className="overflow-hidden">
                ...
                <span className="text-mono-data font-mono-data text-[11px] text-blue-400 block font-medium">
          ...
          <div className="space-y-space-md">
            ...
            <div className="relative p-6 rounded-xl bg-gradient-to-b from-zinc-900/90 to-zinc-950 border border-zinc-800 flex flex-col sm:flex-row items-center gap-6 overflow-hidden">
                ...
                <div 
                  className={`w-24 h-24 rounded-2xl flex items-center justify-center transition-all duration-300 ${
                    isAttested 
                      ...
                      : "bg-blue-950/40 border border-blue-500/30 text-blue-400 animate-pulse"
                  }`}
                >
              ...
              <div className="flex-1 text-center sm:text-left space-y-2">
                <h3 className="text-headline-sm font-headline-sm text-starlight-white leading-tight">
                  {isAttested ? "Hardware Key Attested & Verified" : "Insert hardware token to begin FIDO2 enrollment."}
                </h3>
                <p className="text-body-md font-body-md text-muted-silver">
                  {isAttested 
                    ? "Cryptographic public key successfully pinned to secure local enclave." 
                    : "Touch the gold contact on your YubiKey or security key when the prompt flashes."}
                </p>
                {/* Dynamic Status Bar */}
                <div className="pt-2">
            ...
            <div className="space-y-4 pt-1">
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  <input 
                    className="w-full pl-10 pr-4 py-2.5 bg-zinc-900 border border-zinc-800 rounded-lg text-starlight-white placeholder-zinc-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-body-md font-body-md transition-all duration-150" 
                    id="node-alias" 
                    placeholder="e.g., Station-04-Primary-YubiKey" 
                    type="text" 
                    value={nodeAlias}
              ...
              <div className="space-y-2.5 pt-1">
                <label className="flex items-start gap-3 cursor-pointer group">
                  <input 
                    ...
                    className="mt-0.5 rounded bg-zinc-900 border-zinc-700 text-primary-container focus:ring-primary-container focus:ring-offset-zinc-900" 
                    type="checkbox" 
                  />
                ...
                <label className="flex items-start gap-3 cursor-pointer group">
                  <input 
                    checked={registerBackup} 
                    onChange={(e) => setRegisterBackup(e.target.checked)}
                    className="mt-0.5 rounded bg-zinc-900 border-zinc-700 text-primary-container focus:ring-primary-container focus:ring-offset-zinc-900" 
                    type="checkbox" 
                  />
                  <div className="text-body-md font-body-md text-starlight-white leading-snug">
          ...
          <div className="mt-space-lg pt-4 border-t border-zinc-800/80 flex flex-col sm:flex-row items-center justify-between gap-4">
            <Link 
              href="/admin" 
              className="w-full sm:w-auto px-4 py-2.5 rounded-lg text-body-md font-body-md text-muted-silver hover:text-starlight-white hover:bg-zinc-800/60 transition-colors flex items-center justify-center gap-1.5 order-2 sm:order-1"
            >
            ...
            <div className="w-full sm:w-auto flex items-center gap-3 order-1 sm:order-2">
              ...
              <button 
                onClick={handleAttestAndContinue}
                className="flex-1 sm:flex-initial px-8 py-3 rounded-full bg-primary-container text-white font-headline-sm text-body-md shadow-lg shadow-coral/20 hover:brightness-110 active:scale-95 transition-all flex items-center justify-center gap-2 font-medium" 
                type="button"
              >
