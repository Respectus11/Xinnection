...
import { TriageThread } from "@/data/mockData";
...
export function TriageQueueRow({ thread }: Readonly<TriageQueueRowProps>) {
  ...
  const riskStyles: Record<string, { bgRow: string; dot: string; status: string; tierLabel: string; tierBadge: string; time: string; action: React.ReactNode }> = {
    high: {
  ...
  const style = riskStyles[thread.risk] || riskStyles.low;
    ...
    <tr className={`${style.bgRow}`}>
      <td className="py-3.5 px-space-md">
        <div className="flex items-center gap-2">
          ...
          <span className="font-mono-data text-mono-data font-bold text-deep-midnight">{thread.id}</span>
        </div>
        <span className={`text-mono-data font-mono-data font-medium block mt-0.5 ml-4 ${style.status}`}>
          {thread.risk === 'high' ? 'Unassigned' : 'Assigned'}
        </span>
      </td>
      <td className="py-3.5 px-space-md">
        <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold border ${style.tierBadge}`}>
          {thread.risk === 'high' && <span className="material-symbols-outlined text-sm" style={{ fontVariationSettings: "'FILL' 1" }}>error</span>}
          {style.tierLabel}
        </span>
      </td>
      <td className="py-3.5 px-space-md">
        <div className="flex flex-wrap gap-1">
          ...
          {thread.tags.map((tag) => (
            <span key={tag} className="px-2 py-0.5 rounded-full text-xs font-medium bg-rose-bg text-rose-text">{tag}</span>
          ))}
        </div>
      </td>
      <td className="py-3.5 px-space-md">
      ...
      <td className="py-3.5 px-space-md whitespace-nowrap">
        <span className={`font-mono-data text-mono-data font-bold px-2 py-0.5 rounded ${style.time}`}>{thread.timeWaiting}</span>
      </td>
      <td className="py-3.5 px-space-md whitespace-nowrap">
        {thread.risk === 'high' ? (
